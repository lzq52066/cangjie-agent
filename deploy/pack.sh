...
if [ "$SKIP_BACKEND" -eq 0 ]; then
  ...
  mkdir -p "$SCRIPT_DIR/backend/artifacts"
  cp -f "$JAR" "$SCRIPT_DIR/backend/artifacts/cangjie-start.jar"
  echo ">>> 后端制品已就位：backend/artifacts/cangjie-start.jar ($(du -m "$JAR" | cut -f1) MB)"
fi
